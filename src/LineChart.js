...
class LineChart extends Component{
  constructor(props){
    super(props)
    this.state = {
      xScale: d3.scaleTime().domain([d3.min(this.props.data, d => d.x, d3.max(this.props.data, d => d.x))]).range([0, this.props.width]),
      yScale: d3.scaleLinear().domain([d3.min(this.props.data, d => d.y), d3.max(this.props.data, d => d.y)]).range([this.props.height, 0])
    }
  }
